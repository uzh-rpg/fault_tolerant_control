...
DEFINE_double(vio_landmark_location_weight, 0.0,
              "Weight of the object function enforcing landmark on the ground.");
DEFINE_double(vio_landmark_distance_weight, 5.0,
              "Weight of the object function enforcing landmark on the ground.");              
DEFINE_double(vio_imu_covariance_scale, 1.0,
              "Scale factor of IMU covariance matrix.");
DEFINE_bool(vio_estimated_landmarks_to_frontend, true, 
              "Copy estimated landmark locations to frontend");
DEFINE_bool(vio_optimize_landmark_once, false,
              "Fix landmarks in the map once been solved in a single key frame.");
DEFINE_bool(vio_disable_marginalization, true,
              "Run code under the framework with marginalization but directly discard marginalized parameters");
DEFINE_bool(vio_do_fixation, true,
              "Fix position and yaw of the oldest frame in the sliding window");
    ...
    const double& height_mea)
{
...

  {
    ...
    bool as_keyframe;
    {
      auto t = timers_[Timer::keyframe_decision].timeScope();

      as_keyframe =  nframe_k->isKeyframe();
        ...
        motion_type == VioMotionType::Invalid))
    {
      ...
      constexpr double zero_vel_prior_sigma = 1.0e-3;
      constexpr double max_velocity_for_zero_vel_prior = 1.0;
      constexpr double zero_vel_prior_dampening_factor = 0.01;
    ...
    // Add height_mea to backend -------
    {
      auto t = timers_[Timer::add_height_mea].timeScope();
      backend_.addHeightMea(height_mea);
    }
    ...
    {
      auto t = timers_[Timer::add_observations].timeScope();
...
void CeresBackendInterface::optimizationLoop()
{
  ...
  while (!stop_thread_)
  {
    ...
    // Copy all persistent landmarks from optimizations
      if (FLAGS_vio_estimated_landmarks_to_frontend)
      {
        for (size_t kp_id = 0; kp_id <= landmarks_.numLandmarks(); ++kp_id)
        {

          if (landmarks_.typeAtSlot(kp_id) == LandmarkType::Persistent)
          {
              MapPoint map_landmark_point;
              LandmarkHandle lm_h = landmarks_.getHandleAtSlot(kp_id);
              BackendId landmark_ze_id = createLandmarkId(lm_h);
              if (backend_.getLandmark(landmark_ze_id, map_landmark_point)) 
              {
                landmarks_.p_W_atSlot(lm_h.slot) = map_landmark_point.point.head<3>();        
              }  
          }
        }
      }
