    ...
    const uint32_t num_tracked)
{
  ...
  if (num_tracked < FLAGS_vio_kfselect_numfts_lower_thresh)
  {
    ...
    std::cout <<"Num of frames between keyframes: " << num_between_keyframe << std::endl;
    num_between_keyframe = 0;
    return true;
  }

  num_between_keyframe ++ ;
  ...
  //   // std::cout << "KF Select: Disparity since last KF = " << median_disparity << std::endl;
  ...
  //   // std::cout << "NEW KEYFRAME: kfselect_min_disparity" << std::endl;
  // return true;
