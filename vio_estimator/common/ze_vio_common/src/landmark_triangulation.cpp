...
// -------------------------------------------------------------------------------
void triangulateNewLandmarks(
    const NFrameTable& states,
    const TransformationVector& T_C_B,
    LandmarkTable& landmarks,
    std::vector<LandmarkHandle>& new_landmarks,
    double height_mea)
{

  //for i = each lm_persistent landmark
  for (auto& lm_h : new_landmarks)
  {
    // landmark observation in the last frame
    const LandmarkObs obs_last = landmarks.lastObs(lm_h);
    Bearing p_C = obs_last.f_;
    Vector2 uv = p_C.topRows<2>().array().rowwise() / p_C.bottomRows<1>().array();

    // estimated pose of the last frame
    Transformation T_C_W = T_C_B[obs_last.frame_idx_] * states.T_B_W(obs_last.nframe_handle_);
    // Matrix44 projection = T_C_W.getTransformationMatrix();
    Transformation T_W_C = T_C_W.inverse();
    Matrix33 rotation = T_W_C.getRotationMatrix();
    Vector3 position = T_W_C.getPosition();

    // dcalculate estimated location of the landmark
    Vector3 homocoord(uv(0), uv(1), 1.0);
    Vector3 tmp = rotation.inverse() * homocoord;
    double lambda =  - height_mea / tmp(2);
    Vector2 dxy(tmp(0)*lambda, tmp(1)*lambda);

    Vector3 pos;
    pos << position(0) + dxy(0),
           position(1) + dxy(1),
           position(2) - height_mea;

    landmarks.p_W(lm_h) = pos;   
  }
}
