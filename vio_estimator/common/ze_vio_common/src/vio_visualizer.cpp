...
void VioVisualizer::displayAllLandmarks(
    const LandmarkTable& landmarks,
    const std::vector<LandmarkHandle>& new_lm,
    bool draw_only_persistent,
    bool draw_triangulated)
{
  {
    if(!draw_triangulated)
    {
    ...
    {
      Positions points(3, new_lm.size());
      uint32_t num_points = 0u;
      for (auto& lm : new_lm)
      {
        points.col(num_points++) = landmarks.p_W(lm);
      }
      points.conservativeResize(3, num_points);
      viz_->drawPoints("landmarks", 0, points, ze::Colors::Red,
                      0.05 * FLAGS_vio_viz_marker_scale);
    }
...
void VioVisualizer::publishOdometry(const Odometry &state)
{
  ...
  if (pub_odom_->getNumSubscribers() > 0)
  {
    ...
    Position p = state.T_W_B.getPosition();
    Quaternion q = state.T_W_B.getRotation();
    Vector3 v = state.v_W;
    Vector3 omega = state.omega_B;
...
void VioVisualizer::visualizeCopiedData()
{
  ...
  std::vector<LandmarkHandle> null_lm;
  displayAllLandmarks(landmarks_, null_lm, false, false);
}
