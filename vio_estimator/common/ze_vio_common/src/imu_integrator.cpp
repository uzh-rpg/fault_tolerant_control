    ...
    Vector3& v_W) const
{
  ...
  {    
    Eigen::MatrixXd acc_gyro_tmp = Eigen::MatrixXd::Zero(6,1);
    if (!acc_gyr.col(i).hasNaN())
      acc_gyro_tmp = acc_gyr.col(i);
    
    real_t dt = nanosecToSecTrunc(
                  stamps(i+1) - stamps(i));
    propagate(q, t, v_W,
              acc_gyro_tmp.block<3,1>(0,0),
              acc_gyro_tmp.block<3,1>(3,0),
              dt);
  }
